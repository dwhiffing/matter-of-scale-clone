...
const InstanceLineItem = ({ instance, clickInstance, clickComplete }) => {
  ...
  const autoComplete = isComplete
    ? 100 - instance.autoCompleteProgress()
    : percent
  const onClick = isComplete
    ? () => clickComplete(instance.id)
    : () => clickInstance(instance.id)
...
export default InstanceLineItem
